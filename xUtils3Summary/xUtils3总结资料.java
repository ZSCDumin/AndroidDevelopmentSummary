...
3、初始化
    ...
    public class MainActivity extends AppCompatActivity {
	    ...
	    @ViewInject(R.id.textView) //关联TextView,相当于findViewById
	    private TextView textView; //可以不使用Private

	    @Override
	    protected void onCreate(Bundle savedInstanceState) {
	        super.onCreate(savedInstanceState);
	        x.view().inject(this); //初始化（这句话一定要加上,没加的话TextView的文本显示不出来）
	    }

	    /**
	     * 单击事件
	     * type默认View.OnClickListener.class，故此处可以简化不写，@Event(R.id.bt_main)
	     * 返回值类型和参数类型必须一致（原因请分析源码）
	     */
...
4、使用@Event事件注解(@ContentView, @ViewInject等更多示例参考sample项目)
